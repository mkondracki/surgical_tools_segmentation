...
from sam2.modeling.backbones.lora import mark_only_lora_as_trainable
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.peft:
            ...
            if 'adapter' in self.peft:
                assert not freeze_image_encoder, "Set up freeze_image_encoder to False if you want to use image encoder adapters"
                for n, value in self.named_parameters(): 
                    if "Adapter" in n:
                        value.requires_grad = True

            if 'lora' in self.peft:
                assert not freeze_image_encoder, "Set up freeze_image_encoder to False if you want to apply lora to image encoder"
                # for n, value in self.image_encoder.named_parameters(): 
                for n, value in self.named_parameters(): 
                    if "lora_"  in n.lower():
                        value.requires_grad = True
                        
            if 'neck' in self.peft:
                # for n, value in self.image_encoder.named_parameters(): 
                for n, value in self.named_parameters(): 
                    if "neck" in n.lower():
                        value.requires_grad = True
                # also make the mask_downsample trainable
                for n, value in self.named_parameters(): 
                    if n.lower().startswith("mask_downsample"):
                        value.requires_grad = True
            
            if 'adaptation' in self.peft:
                # for n, value in self.image_encoder.named_parameters(): 
                for n, value in self.named_parameters(): 
                    if "adaptation" in n.lower():
                        value.requires_grad = True
