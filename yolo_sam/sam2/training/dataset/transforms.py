...
class AddRandomGaussianUniformNoise:
    """
    Add Gaussian noise with uniform sigma to each frame's tensor data.
    Args:
        sigma (float): Standard deviation of the Gaussian noise.
        consistent_transform (bool): If True, apply the same noise to all frames.
    """
    def __init__(self, sigma_max, consistent_transform=False):
        self.sigma = random.uniform(0, sigma_max)
    ...
    def __call__(self, datapoint: VideoDatapoint, **kwargs):
        if self.consistent_transform:
            ...
            for img in datapoint.frames:
                if noise is None:
                    noise = torch.randn_like(img.data) * self.sigma
                img.data = img.data + noise
                img.data = torch.clamp(img.data, 0, 1)  # Ensure pixel values are in [0, 1]
        else:
            for img in datapoint.frames:
                noise = torch.randn_like(img.data) * self.sigma
                img.data = img.data + noise
...
class ToTensorAPI:
    def __init__(self, v2=False, single_channel_unnormalize=False):
        self.v2 = v2
        self.single_channel_unnormalize = single_channel_unnormalize
    ...
    def __call__(self, datapoint: VideoDatapoint, **kwargs):
        for img in datapoint.frames:
            ...
            if self.single_channel_unnormalize:
                img.data = img.data[0:1, :, :].multiply(255)
        return datapoint
...
class NormalizeFluoro:
    def __call__(self, datapoint, **kwargs):
        for img in datapoint.frames:
            input = img.data
            input -= input.min()
            assert input.max() > 1, "The input image is already normalized"
            input /= input.max()
            img.data = input
        return datapoint
