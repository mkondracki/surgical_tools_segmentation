...
class PNGRawDataset(VOSRawDataset):
    ...
    def get_idx(self, video_name):
        """
        Get the index of the video name in the dataset.
        """
        if self.single_object_mode:
    ...
    def get_video(self, idx):
        ...
        all_frames = sorted(
            glob.glob(os.path.join(video_frame_root, "*.png")) +
            glob.glob(os.path.join(video_frame_root, "*.jpg")) +
            glob.glob(os.path.join(video_frame_root, "*.jpeg"))
        )
    ...
    def get_video_only(self, idx):
        """
        Return only the VOSVideo object for the given index, without loading any segment_loader.
        """
        video_name = self.video_names[idx]
