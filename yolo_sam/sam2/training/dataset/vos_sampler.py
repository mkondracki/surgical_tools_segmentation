...
class EvalSampler(VOSSampler):
    ...
    def sample_video_only(self, video):
        """
        Sample all frames from the video, object_ids is set to None.
        """
        if self.sort_frames:
            frames = sorted(video.frames, key=lambda x: x.frame_idx)
        else:
            frames = video.frames
        return SampledFramesAndObjects(frames=frames, object_ids=None)
