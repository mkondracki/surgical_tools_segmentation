...
class Trainer:
    ...
    def save_checkpoint(self, epoch, checkpoint_names=None):
        ...
        checkpoint = {
            ...
            "loss_values": self.loss.state_dict(),
            "steps": self.steps,
    ...
    def save_best_val_loss_checkpoint(self, epoch, val_loss):
        if not hasattr(self, "best_val_loss"):
            self.best_val_loss = float("inf")  # Initialize best_val_loss

        if val_loss < self.best_val_loss:
            self.best_val_loss = val_loss
            best_model_path = os.path.join(self.checkpoint_conf.save_dir, "checkpoint_best_val_loss.pt")
            state_dict = unwrap_ddp_if_wrapped(self.model).state_dict()
            checkpoint = {
                "model": state_dict,
                "optimizer": self.optim.optimizer.state_dict(),
            ...
            with g_pathmgr.open(best_model_path, "wb") as f:
    ...
    def _step(
        ...
        outputs = model(batch)
        viz_pred = outputs[0]['pred_masks']
        viz_pred_0 = viz_pred[0][0]
        targets = batch.masks
        targets_0 = targets[0][:1].int()
        img_input = batch.img_batch.squeeze(1)
        ...
        pred_masks_high_res = [outputs[i]['pred_masks_high_res'] for i in range(len(outputs))]  # Shape: [56, 1, 512, 512]
        
        # if phase == 'train':
        #     save_side_by_side_masks_imgs(
        #         batch.masks.squeeze(0).int(),
        #         batch.img_batch.squeeze(),
        #         pred_masks_high_res[0],
        #         f"/home/samba/320281459/code/sam2/training/dataset/synthetic_dataset/tensor_viz/{phase}"
        #     )
        
        if phase == 'val':
            # Select the first image in the batch for logging
            input_images = batch.img_batch.squeeze()  # Shape: [32, 3, 512, 512]
            segmentations = pred_masks_high_res  # Shape: [56, 512, 512]
            gt = batch.data.data.masks.squeeze()


            
            # # Log a limited number of images and their corresponding masks
            # self.logger.tb_logger.log_images(
            #     images=input_images,  # Input image (C, H, W)
            #     segmentations=segmentations,  # Predicted mask (H, W)
            #     gt=gt,
            #     step=self.steps[Phase.VAL],
            # )
    ...
    def run_val(self):
        ...
        if 'val' in self.loss.keys(): 
            self.save_best_val_loss_checkpoint(self.epoch + 1, outs[f"Losses/{Phase.VAL}_val_loss"])
            # print("SAVED BEST VAL CHECKPOINT")
        else: 
            print("COULDN'T SAVE BEST VAL CHECKPOINT SINCE VAL IS NOT IN THE LOSS KEYS")
    ...
    def has_freezed_params(self):
        return self.model.peft is not None or \
                self.model.freeze_image_encoder is not False
...
def save_side_by_side_masks_imgs(
    masks,           # torch.Size([6, 768, 768])
    imgs,            # torch.Size([2, 3, 768, 768])
    pred_masks,      # torch.Size([6, 1, 768, 768])
    save_dir,
    filename="side_by_side.png"
):
    """
    Save a grid of (input image, GT mask, predicted mask, binarized prediction) side by side.
    - masks: [6, 768, 768] (GT masks)
    - imgs: [2, 3, 768, 768] (input images)
    - pred_masks: [6, 1, 768, 768] (predicted masks)
    """
    ...
    img = imgs[0]  # [3, 768, 768]
    mask_triplets = []
    for i in range(masks.shape[0]):
        gt_mask = masks[i].unsqueeze(0).float()
        pred_mask = pred_masks[i][0].unsqueeze(0).float()
        ...
        pred_mask_bin = (pred_mask_norm > 0.5).float()
        # Convert masks to 3 channels for visualization
        gt_mask_rgb = gt_mask.repeat(3, 1, 1)
        pred_mask_rgb = pred_mask_norm.repeat(3, 1, 1)
        pred_mask_bin_rgb = pred_mask_bin.repeat(3, 1, 1)
        # Stack: [img | gt_mask | pred_mask | pred_mask_bin]
        quad = torch.cat([img, gt_mask_rgb, pred_mask_rgb, pred_mask_bin_rgb], dim=2)  # [3, 768, 768*4]
        mask_triplets.append(quad)
    # Make a grid: each row is a quad
    grid = torch.cat(mask_triplets, dim=1)  # [3, 768*6, 768*4]
